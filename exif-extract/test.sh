...
body="{\"endpoint\": \"http://zenko-cloudserver.default.svc.cluster.local:80\", \"credentials\": {\"accessKeyId\": \"${AWS_ACCESS_KEY_ID}\", \"secretAccessKey\": \"${AWS_SECRET_ACCESS_KEY}\" }, \"Bucket\": \"${TEST_BUCKET}\", \"Key\": \"test1.jpg\", \"tags\": {}, \"param\": \"longitude=gps.GPSLongitude,latitude=gps.GPSLatitude\"}"
echo $body
result=`fission fn test --name ${FUNC} -H "Content-Type: application/json" --method ${METHOD} -b "${body}"`
ifexit
echo $result |grep latitude
ifexit
